...
st.set_page_config(page_title="Video GPT", page_icon=":sparkles:", layout="wide")
ai_model = genai.GenerativeModel(st.secrets["model_name"])

def main():
    st.markdown(
        """
        <h2 style='text-align: center;'>Video AI: Powered by Theta CloudGPT AI</h2>
        """,
    ...
    with left_column:
        ...
        if uploaded_file is not None:
            ...
            # Display the video
            st.video(img, caption="Uploaded Video", use_column_width=True)
    ...
    if uploaded_file is not None:
        ...
        with right_column:
            st.markdown(
                """
                <h4 style='text-align: center; margin-top: 20px;'>Upload an video to start asking questions</h4>
                """,
                unsafe_allow_html=True
            )
